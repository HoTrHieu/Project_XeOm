...
module.exports.GetData = async (req, res) => {
    ...
    let day = timeData.substr(6, 8);
    let week = timeData.substr(timeData.length - 1);
    let timeCompare = year + '-' + month + '-' + day;
    console.log("timeData : " + timeCompare)
    timeCompare = new Date(timeCompare).toDateString()
    ...
    chuyendis.map(chuyendi => {
        taixes.map(taixe => {
            if (type === "D") {
                ...
                let rgxp = new RegExp(timeCompare, "g")
                if (
...
module.exports.GetByPhone = async (req, res) => {
    ...
    chuyendis.map((chuyendi) => {
        taixes.map((taixe) => {
            if (type === "D") {
                let time = new Date(chuyendi.TinhTrang["time"]) + ""
                console.log("time db: " + time + "-type : " + typeof (time))
                let rgxp = new RegExp(timeCompare, "g")
                if (time.match(rgxp)) {
                    console.log("giong")
                ...
                let time = new Date(chuyendi.TinhTrang["time"])
                    ...
                    .slice(0, 8)
                    .replace(/[-T:]/g, "")
                console.log("time req: " + timeData + "- time db: " + time)
                if (taixe.SoDienThoai == chuyendi.SDTTaiXe && time === timeData) {
                    Similarphone.push({ SimilarPhone: { chuyendi, taixe, time } });
                }
            } else if (type === "W") {
...
module.exports.GetGroupByPhone = async (req, res) => {
    ...
    let timeCompare = year + '-' + month + '-' + weekorday;
    console.log("timeData : " + timeCompare)
    timeCompare = new Date(timeCompare).toDateString()
    console.log("timeCompare: " + timeCompare)
    ...
    let rgxp = new RegExp(timeCompare)
    console.log("rgxp: " + rgxp)
    const chuyen = await ChuyenDi.find({TinhTrang: {time: {$regex: /^Fri/}}})
    chuyen.map(cd=>{
        console.log(cd.TinhTrang.time + "-- matchhhh")
    })  

    let taixes;
    if (type === "D") {
        /* let timeFind = year + "-" + month + "-" + weekorday;
        console.log("timefind : " + timeFind) */
